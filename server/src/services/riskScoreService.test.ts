...
assert(noDisasters.nearbyDisasters.length === 0, 'No nearby disasters');
assert(
...
// 2. Disaster far away → 100 (no penalty; outside 50 km)
...
assert(farResult.nearbyDisasters.length === 0, 'Far disaster not in nearby list');
console.log('✓ Disaster far away → score 100');

// 3. Disaster within 50 km → score drops (one penalty = 25)
...
assert(nearbyResult.nearbyDisasters.length === 1 && nearbyResult.nearbyDisasters[0].title === 'Flood declaration', 'Nearby disaster in list');
assert(
