...
export async function getFemaDisasters(limit = 50) {
  const url =
    "https://www.fema.gov/openfema-data-page/disaster-declarations-summaries-v2"; // keep for reference

  const api =
    `https://www.fema.gov/api/open/v2/DisasterDeclarationsSummaries?$top=${limit}&$orderby=declarationDate desc`;
